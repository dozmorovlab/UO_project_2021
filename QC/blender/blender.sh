...
/usr/bin/time -v sort -m -k1,1d -k5,5d <(gunzip -c $primary1) <(gunzip -c $primary2) <(gunzip -c $primary3) | gzip -c > "${INPUT}/primary_merged/merged_nodups.txt.gz"
...
/usr/bin/time -v sort -m -k1,1d -k5,5d <(gunzip -c $CR1) <(gunzip -c $CR2) | gzip -c > "${INPUT}/CR_merged/merged_nodups.txt.gz"
...
/usr/bin/time -v sort -m -k1,1d -k5,5d <(gunzip -c $LiverMet1) <(gunzip -c $LiverMet2) | gzip -c > "${INPUT}/livermet_merged/merged_nodups.txt.gz"
